...
data class StatisticComparison(val first: Statistics, val second: Statistics) {
    ...
    val avgBedTimeDiffHours: Float get() = if (second.isEmpty) 0.0f else second.averageBedTime.diffHours(first.averageBedTime)
    val avgWakeUpTimeDiffHours: Float get() = if (second.isEmpty) 0.0f else second.averageWakeUpTime.diffHours(first.averageWakeUpTime)
